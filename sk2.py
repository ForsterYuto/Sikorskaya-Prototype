...
def main():
    ...
    if selected_item == "おみくじ":
        ...
        st.write("シコルスカヤ" + sk.__version__  + "：" + name + "同志の運勢は『" + str(unsei[num]) + "』です！")
    
    elif selected_item == "運勢ポイント":
        num=random.randrange(1,7875000000)
        point=random.randrange(1,999)
        st.write("シコルスカヤ" + sk.__version__  + "：同志"+name+"の運勢ポイントは"+str(point)+"プロレタリアポイント、\n 世界第"+str(num)+"位です！")
    
    elif selected_item == "ソーシャルクレジット":
        num=random.randrange(0,125459999)
        point=random.randrange(0,9999999)
        st.write("シコルスカヤ" + sk.__version__  + "：現在の同志" + name + "のソーシャルクレジットは" + str(point) + "ポイント、日本第" + str(num) +"位です。")
    
    elif selected_item == "人権ポイント":
        num=random.randrange(0,125459999)
        point=random.randrange(0,9999999)
        st.write("シコルスカヤ" + sk.__version__  + "：現在の同志" + name + "の人権ポイントは" + str(point) + "ポイント、日本第" + str(num) +"位です。\nあるいは最高位人権道徳者を目指しなさい。")
